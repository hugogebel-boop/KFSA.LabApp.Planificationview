...
type Status = 'a_faire'|'encours'|'termine'|'valide'
type Item = { id:string, affaire:string, ech:string, test:string, status:Status, resource?:string }
...
const LABEL: Record<Status,string> = { a_faire:'À faire', encours:'En cours', termine:'Terminé', valide:'Validé' }
const TONE: Record<Status,string> = {
  a_faire:'bg-stone-100 text-stone-800 border border-stone-200',
  encours:'bg-emerald-100 text-emerald-800 border border-emerald-200',
  termine:'bg-amber-100 text-amber-800 border border-amber-200',
  valide:'bg-indigo-100 text-indigo-800 border border-indigo-200',
}

export function KanbanView(){
  const cols: Record<Status, Item[]> = {
    a_faire: DATA.filter(r=>r.status==='a_faire'),
    encours: DATA.filter(r=>r.status==='encours'),
    termine: DATA.filter(r=>r.status==='termine'),
    valide:  DATA.filter(r=>r.status==='valide'),
  }
  const order: Status[] = ['a_faire','encours','termine','valide']
        ...
        return (
          <Card key={k}>
            <CardHeader className="flex items-center justify-between">
              <CardTitle className="capitalize">{LABEL[k]}</CardTitle>
              <Badge className={TONE[k]}>{list.length}</Badge>
            </CardHeader>
            <CardContent className="flex flex-col gap-3">
              {list.map(r => (
                <div key={r.id} className="rounded-xl border border-emerald-100 bg-white/80 p-3 shadow-sm">
                  <div className="text-sm font-semibold text-emerald-950">{r.test}</div>
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-emerald-900/80">
                    <Badge className="bg-emerald-100 text-emerald-800 border border-emerald-200">Affaire {r.affaire}</Badge>
                    <Badge className="bg-teal-100 text-teal-800 border border-teal-200">Éch. {r.ech}</Badge>
                    {r.resource && <Badge>{r.resource}</Badge>}
                  </div>
                </div>
              ))}
              {list.length===0 && (
                <div className="rounded-lg border border-dashed border-emerald-200/70 bg-white/30 p-4 text-center text-xs text-emerald-900/60">
