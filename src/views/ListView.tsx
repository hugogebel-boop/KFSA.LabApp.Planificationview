...
type Row = { id:string, affaire:string, ech:string, test:string, status:Status, resource?:string, date?:string }
const DATA: Row[] = [
...
export function ListView(){
  ...
  const filtered = useMemo(()=>{
    ...
    return DATA.filter(r=>!s || r.affaire.toLowerCase().includes(s) || r.ech.toLowerCase().includes(s) || r.test.toLowerCase().includes(s) || LABEL[r.status].toLowerCase().includes(s))
  },[q])
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="mb-3 flex items-center gap-2 rounded-lg border border-emerald-100 bg-white/70 px-3 py-2">
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Rechercher (Affaire, Éch., Test, Statut)" className="h-7 w-full bg-transparent text-sm outline-none"/>
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-emerald-50/60 text-emerald-900">
              <tr>
                ...
                <th className="px-3 py-2 text-left font-medium">Éch.</th>
            ...
            <tbody>
              {filtered.map(r=> (
                <tr key={r.id} className="border-t">
                  <td className="px-3 py-2">{r.affaire}</td>
                  <td className="px-3 py-2">{r.ech}</td>
                  <td className="px-3 py-2">{r.test}</td>
                  <td className="px-3 py-2"><span className={`inline-flex rounded-md px-2 py-0.5 ${TONE[r.status]}`}>{LABEL[r.status]}</span></td>
                  <td className="px-3 py-2">{r.resource ?? '—'}</td>
                  <td className="px-3 py-2">{r.date ?? '—'}</td>
                </tr>
              ))}
              {filtered.length===0 && (
                <tr><td colSpan={6} className="px-3 py-6 text-center text-emerald-900/70">Aucun résultat.</td></tr>
              )}
            </tbody>
          </table>
        </div>
